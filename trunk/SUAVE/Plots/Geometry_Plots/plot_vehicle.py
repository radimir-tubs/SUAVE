...
## @ingroup Plots-Geometry_Plots
def plot_vehicle(vehicle, save_figure = False, plot_wing_surface = True, plot_control_points = True, save_filename = "Vehicle_Geometry"):     
    ...
    wing_alpha_val  = 1    
    plot_wings(axes,VD,plot_wing_surface,wing_face_color,wing_edge_color,wing_alpha_val)  
...
def plot_wings(axes,VD,plot_wing_surface,face_color,edge_color,alpha_val): 
    ...
    if plot_wing_surface:
        n    = VD.n_sp
        for i in range(n):  
            X = [VD.XA1_ts[i],VD.XB1_ts[i],VD.XB2_ts[i],VD.XA2_ts[i]]
            Y = [VD.YA1_ts[i],VD.YB1_ts[i],VD.YB2_ts[i],VD.YA2_ts[i]]
            Z = [VD.ZA1_ts[i],VD.ZB1_ts[i],VD.ZB2_ts[i],VD.ZA2_ts[i]]  
...
def plot_propulsor(axes,VD,propulsor,propulsor_face_color,propulsor_edge_color,propulsor_alpha,tessellation = 24):
    ...
    for energy_component,component in propulsor.items():
        if isinstance(component,Energy_Component):
            if  (energy_component == 'propeller') or (energy_component == 'rotor'):   
                ...
                # Generate Nacelle Geoemtry
                nac_geo = generate_nacelle_points(VD,component,propulsor,propulsor.tag,tessellation)
...
def plot_propeller_geometry(axes,prop,propulsor,propulsor_name):
    ...
    if propulsor_name == 'propeller': 
        try:
            ta1 = -propulsor.propeller_thrust_angle 
        except: 
            ta1 = -propulsor.thrust_angle
    elif propulsor_name == 'rotor': 
        try:
            ta1 = -propulsor.rotor_thrust_angle 
        except: 
            ta1 = -propulsor.thrust_angle
    else:
        ta1= -propulsor.thrust_angle 
      
    # Thrust angle of rotating blades  
    if propulsor_name == 'propeller':  
            ta2    = -propulsor[propulsor_name].thrust_angle 
    elif propulsor_name == 'rotor':  
            ta2    = -propulsor[propulsor_name].thrust_angle  
    ...
    for n_p in range(num_props):  
        ...
        for i in range(num_B):   
            ...
            trans_3 = np.array([[np.cos(thrust_angle),0 , -np.sin(thrust_angle)],
                           [0 ,  1 , 0] ,
                           [np.sin(thrust_angle) , 0 , np.cos(thrust_angle)]])
